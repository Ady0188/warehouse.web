...
{
    public async Task<ExportFileResult> Export(List<OrderResponse> items)
    {
        ...
        // Load the template
        var templateFilePath = Path.Combine("Templates", "Orders.xltx");
        ...
        var title = $"Ордеры{storeName}{managerName}{agentName}";
        if (types.Count() == 1)
